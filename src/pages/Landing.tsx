...
const Landing = () => {
  ...
  useEffect(() => {
    ...
    const isPostLogout = url.searchParams.get("postLogout") === "1";
    if (!isPostLogout) return;

    const action = sessionStorage.getItem("creatoros.auth.postLogoutAction");
    sessionStorage.removeItem("creatoros.auth.postLogoutAction");

    url.searchParams.delete("postLogout");
  ...
  const handleLogin = () => {
    if (isAuthenticated) {
      sessionStorage.setItem("creatoros.auth.postLogoutAction", "login");
      logout();
      return;
    }
    void login("", "");
  ...
  const handleRegister = () => {
    if (isAuthenticated) {
      sessionStorage.setItem("creatoros.auth.postLogoutAction", "register");
      logout();
      return;
    }
    void register("", "");
